...
; Código 22:    Control de un LED con un SW de usuario con debounce
...
loop
    ...
    ; Parámetros para subrutina Delay
        MOVW    R0, #0x4154
        MOVT    R0, #0x0003                     ; n = 213,332 (t = 80ms)

        BL      Delay                           ; Retardo de rebote (debounce)

    ; Leer nuevamente el estado del SW1 de la tarjeta de desarrollo
        LDR     R6, [R4]                        ; R6 = [GPIO_PORTJ_AHB_DATA_R]
        CMP     R6, #0x00                       ; Condición (SW1 = on)
        BNE     loop
