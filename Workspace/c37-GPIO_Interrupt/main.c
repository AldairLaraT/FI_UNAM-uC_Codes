...
void GPIO_PortJ_Init(void) {
    ...
    GPIO_PORTJ_AHB_IS_R &= ~0x01;                                                                   /* PortJ[0] => IS: Interrupt Sense -> Edge-sensitive */
    GPIO_PORTJ_AHB_IBE_R &= ~0x01;                                                                  /* PortJ[0] => IBE: Interrupt Both Edges -> Controlled by GPIO_IEV */
    GPIO_PORTJ_AHB_IEV_R &= ~0x01;                                                                  /* PortJ[0] => IEV: Interrupt Event -> Falling edge */

    /** Limpiar la bandera de interrupción. */
    GPIO_PORTJ_AHB_ICR_R |= 0x01;                                                                   /* PortJ[0] => IC: Interrupt Clear -> Interrupt cleared */

    /** Desenmascarar la interrupción local. */
    GPIO_PORTJ_AHB_IM_R |= 0x01;                                                                    /* PortJ[0] => IME: Interrupt Mask Enable -> Interrupt unmasked */
...
void SysTick_Init_OneShot(uint32_t SysTick_Reload) {
    ...
    /** Limpiar el valor de carga/inicio del SysTick para deshabilitar el contador en el siguiente
     *  ciclo. */
