 ...
 *
 * Tema 09:     Periféricos
 * Código 51:   Módulo temporizador de propósito general (GPTM): 16bits, one-shot
...
void GPIO_PortJ_Handler(void) {
    ...
    if (!(GPIO_PORTJ_AHB_DATA_R & 0x01)) {                                                          /* IF (SW1 -> on) */
        ...
        /** 8.  Si no se utiliza interrupción, monitorear (por polling) la bandera de interrupción
         *      cruda del time-out. */
    ...
    if (!(GPIO_PORTJ_AHB_DATA_R & 0x02)) {                                                          /* IF (SW2 -> on) */
        ...
        /** 8.  Si no se utiliza interrupción, monitorear (por polling) la bandera de interrupción
         *      cruda del time-out. */
        while (!(TIMER0_RIS_R & TIMER_RIS_TBTORIS)) {}                                              /* GPTM0 => TBTORIS: GPTM Timer B Time-Out Raw Interrupt -> Timer B has timed out */
