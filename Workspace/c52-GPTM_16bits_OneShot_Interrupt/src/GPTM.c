 ...
 *
 * Tema 09:     Periféricos
 * Código 52:   Módulo temporizador de propósito general (GPTM): 16bits, one-shot, interrupción
 * Descripción: Código en lenguaje C que utiliza los temporizadores TIMER0A y TIMER0B en
 *              configuración individual (16 bits) y modo one-shot, empleando interrupciones por
 *              time-out, para generar retardos al presionar un botón (SW1 o SW2) y conmutar los
 *              LED de usuario (D1 o D2) de la tarjeta de desarrollo.
 *
...
void GPTM0B_OneShot_Init(uint16_t LoadValue, uint8_t PrescalerValue) {
    ...
    /** 6.  Si se utiliza interrupción, desenmascarar la interrupción local. */
    TIMER0_IMR_R |= 0x0100;                                                                         /* GPTM0 => TBTOIM: GPTM Timer B Time-Out Interrupt Mask -> Interrupt unmasked */
    ...
    /** Establecer el nivel de prioridad de la IRQ. */
    NVIC_PRI5_R = ((NVIC_PRI5_R & ~0x000000E0) | (1 << 5));                                         /* IRQ_20 (16/32-Bit Timer 0B) => INTA: Interrupt Priority -> Cleared and set 1 */

    /** Habilitación de la IRQ en el NVIC. */
    NVIC_EN0_R |= (1 << (20 - 0));                                                                  /* IRQ_20 (16/32-Bit Timer 0B) => INT: Interrupt Enable -> Enabled */
